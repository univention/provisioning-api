...
class UDMMessagingService(univention.admin.uldap.access):
    def __init__(self, port: UDMMessagingPort):
        super().__init__(
            port=udm_listener_settings.ldap_port,
            start_tls=2 if udm_listener_settings.tls_mode.lower() == "on" else 0,
            base=udm_listener_settings.ldap_base_dn,
            binddn=udm_listener_settings.ldap_host_dn,
            bindpw=udm_listener_settings.ldap_password,
        )

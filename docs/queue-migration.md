...
INFO: No messages in sealed stream, completing migration
...
See `backends/tests/test_nats_mq_migration.py` for comprehensive unit tests covering:
- Stream creation
- Stream updates
- Migration triggering
- Sealed stream handling
- Empty stream migration completion
- Idempotent sealing
...
1. Install old version (WorkQueue mode)
2. Create test users to generate messages
3. Upgrade to new version (Interest mode)
4. Verify:
   - Migration completes automatically
   - No messages lost
   - Multiple dispatchers receive same messages

## References

- [Issue #96](https://git.knut.univention.de/univention/dev/projects/provisioning/-/issues/96): Original issue for multi-dispatcher support
- [MR #287](https://git.knut.univention.de/univention/dev/projects/provisioning/-/merge_requests/287): POC implementation

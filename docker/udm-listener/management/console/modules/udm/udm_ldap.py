    ...
    def reload_cache(cls, module):
        ...
        if module_cache:
            ...
            for attr in module_cache:
                MODULE.info(
                    "    %s and with it: %r" % (attr, module_cache[attr]["attributes"])
                )
...
class UserWithoutDN(UMCError):
    ...
    def _error_msg(self):
        yield _("The LDAP DN of the user %s could not be determined.") % (
            self._username,
        )
        yield _("The following steps can help to solve this problem:")
        yield " * " + _(
            "Ensure that the LDAP server on this system is running and responsive"
        )
        yield " * " + _(
            "Make sure the DNS settings of this server are correctly set up and the DNS server is responsive"
        )
        if not self._is_master:
            yield " * " + _(
                "Check the join status of this system by using the domain join UMC module"
            )
        ...
        yield _(
            "If the problem persists additional hints about the cause can be found in the following log file(s):"
        )
...
class ObjectDoesNotExist(UMCError):
    ...
    def _error_msg(self):
        if self._ldap_object_exists():
            ...
            yield _(
                "If the problem persists please try to relogin into Univention Management Console."
            )
        else:
...
class UDM_Module(object):
    ...
    def __init__(
        self, module, force_reload=False, ldap_connection=None, ldap_position=None
    ):
    ...
    def get_ldap_connection(self):
        if get_bind_function():
            try:
                self.ldap_connection, po = get_user_connection(
                    bind=get_bind_function(), write=True
                )
            except (LDAPError, udm_errors.ldapError):
                self.ldap_connection, po = get_user_connection(
                    bind=get_bind_function(), write=True
    ...
    def load(self, module=None, template_object=None, force_reload=False):
        ...
        try:
            self.module = _module_cache.get(
                module, None, force_reload, *self.get_ldap_connection()
            )  # FIXME: template_object not used?!
    ...
    def _map_properties(self, obj, properties):
        ...
        def _tmp_cmp(i):
            ...
            if (
                i[0] == "network"
            ):  # must be set before ip, dhcpEntryZone, dnsEntryZoneForward, dnsEntryZoneReverse
                return ("\x01", i[1])
            if i[0] in (
                "ip",
                "mac",
            ):  # must be set before dnsEntryZoneReverse, dnsEntryZoneForward
                return ("\x02", i[1])
            return i
        ...
        for property_name, value in sorted(properties.items(), key=_tmp_cmp):
            ...
            if isinstance(value, (tuple, list)) and property_obj.multivalue:
                ...
                for ival in value:
                    try:
                        ...
                        raise UMC_Error(
                            _(
                                "The property %(property)s has an invalid value: %(value)s"
                            )
                            % {"property": property_obj.short_description, "value": exc}
                        )
                if (
                    subResults
                ):  # empty list represents removing of the attribute (handlers/__init__.py def diff)
                    MODULE.info("Setting of property ignored (is empty)")
            ...
            # otherwise we have a single value
                ...
                try:
                    ...
                    raise UMC_Error(
                        _("The property %(property)s has an invalid value: %(value)s")
                        % {"property": property_obj.short_description, "value": exc}
                    )
    ...
    def create(self, ldap_object, container=None, superordinate=None):
        ...
        if container:
            ...
            if hasattr(self.module, "policy_position_dn_prefix"):
                container = "%s,cn=policies,%s" % (
                    self.module.policy_position_dn_prefix,
                    ldap_position.getBase(),
                )
            elif (
                hasattr(self.module, "default_containers")
                and self.module.default_containers
            ):
                container = "%s,%s" % (
                    self.module.default_containers[0],
                    ldap_position.getBase(),
                )
            else:
        ...
        if superordinate:
            _superordinate, mod = get_obj_module(
                self.name, superordinate, ldap_connection
            )
            if not mod:
        ...
        obj = self.module.object(
            None, ldap_connection, ldap_position, superordinate=superordinate
        )
        try:
            obj.open()
            ...
            if "$options$" in ldap_object:
                options = [
                    option
                    for option in ldap_object["$options$"].keys()
                    if ldap_object["$options$"][option] is True
                ]
                for option_name, option_def in AppAttributes.data_for_module(
                    self.name
                ).items():
            ...
            if "$policies$" in ldap_object:
                obj.policies = reduce(
                    lambda x, y: x + y, ldap_object["$policies$"].values(), []
                )
            ...
            MODULE.warn(
                "Failed to create LDAP object: %s: %s" % (e.__class__.__name__, str(e))
            )
    ...
    def move(self, ldap_dn, container):
        ...
        ldap_connection, ldap_position = self.get_ldap_connection()
        superordinate = udm_objects.get_superordinate(
            self.module, None, ldap_connection, ldap_dn
        )
        obj = self.module.object(
            None,
            ldap_connection,
        ...
        try:
            ...
            MODULE.warn(
                "Failed to move LDAP object %s: %s: %s"
                % (ldap_dn, e.__class__.__name__, str(e))
            )
            UDM_Error(e).reraise()
    ...
    def remove(self, ldap_dn, cleanup=False, recursive=False):
        ...
        try:
            ...
            MODULE.warn(
                "Failed to remove LDAP object %s: %s: %s"
                % (ldap_dn, e.__class__.__name__, str(e))
            )
            UDM_Error(e).reraise()
    ...
    def modify(self, ldap_object):
        ...
        ldap_connection, ldap_position = self.get_ldap_connection()
        superordinate = udm_objects.get_superordinate(
            self.module, None, ldap_connection, ldap_object["$dn$"]
        )
        MODULE.info(
            "Modifying object %s with superordinate %s"
            % (ldap_object["$dn$"], superordinate)
        )
        obj = self.module.object(
            None,
            ldap_connection,
            ldap_position,
            dn=ldap_object.get("$dn$"),
            superordinate=superordinate,
        )
        del ldap_object["$dn$"]
        ...
        try:
            ...
            MODULE.warn(
                "Failed to modify LDAP object %s: %s: %s"
                % (obj.dn, e.__class__.__name__, str(e))
            )
            UDM_Error(e).reraise()
    ...
    def search(
        ...
        if attribute in [None, "None"] and filter:
            ...
            filter_s = self._object_property_filter(
                attribute, value, hidden, allow_asterisks
            )

        MODULE.info(
            "Searching for LDAP objects: container = %s, filter = %s, superordinate = %s"
            % (container, filter_s, superordinate)
        )
        result = None
        try:
            ...
            if simple and self.allows_simple_lookup():
                ...
                if lookup_filter is None:
                    ...
                    if simple_attrs is not None:
                        result = ldap_connection.search(
                            filter=six.text_type(lookup_filter),
                            base=container,
                            scope=scope,
                            sizelimit=sizelimit,
                            attr=simple_attrs,
                        ...
                        result = ldap_connection.searchDn(
                            filter=six.text_type(lookup_filter),
                            base=container,
                            scope=scope,
                            sizelimit=sizelimit,
                ...
                if self.module:
                    ...
                    if (
                        serverctrls
                        and "serverctrls" in getfullargspec(self.module.lookup).args
                    ):  # not every UDM handler supports serverctrls
                        kwargs["serverctrls"] = serverctrls
                        kwargs["response"] = response
                    result = self.module.lookup(
                        None,
                        ldap_connection,
                        filter_s,
                        base=container,
    ...
    def get(self, ldap_dn=None, superordinate=None, attributes=[]):
        ...
        try:
            ...
            if (
                isinstance(exc, udm_errors.noObject)
                and superordinate
                and not ldap_connection.get(superordinate)
            ):
                raise SuperordinateDoesNotExist(superordinate)
    ...
    def get_property(self, property_name):
        ...
        return getattr(self.module, "property_descriptions", {}).get(
            property_name, None
        )
    ...
    def help_link(self):
        ...
        if isinstance(help_link, dict):
            ...
            help_link = (
                "https://docs.software-univention.de/%(lang)s-%(version)s.html#%(section)s"
                % defaults
            )
        return help_link
    ...
    def columns(self):
        return [
            {
                "name": key,
                "label": self.module.property_descriptions[key].short_description,
            }
            for key in getattr(self.module, "columns", [])
        ]
    ...
    def subtitle(self):
        ...
        descr = getattr(
            self.module, "short_description", getattr(self.module, "module", "")
        )
        colon = descr.find(":")
        if colon > 0:
    ...
    def identifies(self):
        ...
        for key, prop in getattr(self.module, "property_descriptions", {}).items():
            if prop.identifies:
                MODULE.info(
                    "The property %s identifies to module objects %s" % (key, self.name)
                )
                return key
        return None
    ...
    def child_modules(self):
        ...
        for child in children:
            mod = UDM_Module(
                child, ldap_connection=ldap_connection, ldap_position=ldap_position
            )
            if not mod.module:
                continue
    ...
    def has_tree(self):
        if not getattr(self.module, "childmodules", None):
            return False
        return all(
            getattr(udm_modules.get(mod), "childmodules", None)
            for mod in self.module.childmodules
        )
    ...
    def default_search_attrs(self):
        return [
            key
            for key, prop in self.module.property_descriptions.items()
            if prop.include_in_default_search
        ]

    def obj_description(self, obj):
        description = None
        description_property_name = ucr.get(
            "directory/manager/web/modules/%s/display" % self.name
        )
    ...
    def password_properties(self):
        ...
        passwords = []
        for key, prop in getattr(self.module, "property_descriptions", {}).items():
            if udm_syntax.is_syntax(
                prop.syntax, udm_syntax.passwd
            ) or udm_syntax.is_syntax(prop.syntax, udm_syntax.userPasswd):
    ...
    def properties(self, position_dn):
        ...
        ldap_connection, ldap_position = self.get_ldap_connection()
        props = [
            {"id": "$dn$", "type": "HiddenInput", "label": "", "searchable": False}
        ]
        for key, prop in list(
            getattr(self.module, "property_descriptions", {}).items()
        ):
            if key == "filler":
            ...
            if prop.base_default is not None:
                if isinstance(prop.base_default, (list, tuple)):
                    if (
                        prop.multivalue
                        and prop.base_default
                        and isinstance(prop.base_default[0], (list, tuple))
                    ):
            ...
            if prop.nonempty_is_default and "default" not in item:
                ...
                try:
                    item["default"] = [
                        x["id"]
                        for x in read_syntax_choices(
                            _get_syntax(prop.syntax.name),
                            ldap_connection=ldap_connection,
                            ldap_position=ldap_position,
                        )
                        if x["id"]
            ...
            props.append(item)
        props.append(
            {"id": "$options$", "type": "WidgetGroup", "widgets": self.get_options()}
        )
        props.append(
            {
                "id": "$references$",
    ...
    def operations(self):
        ...
        return getattr(
            self.module, "operations", ["add", "edit", "remove", "search", "move"]
        )
    ...
    def policies(self):
        ...
        if not policyTypes and self.childs:
            ...
            policyTypes = [
                x
                for x in udm_modules.modules
                if x.startswith("policies/") and x != "policies/policy"
            ]

        policies = []
        for policy in policyTypes:
            module = UDM_Module(
                policy, ldap_connection=ldap_connection, ldap_position=ldap_position
            )
            policies.append(
                {
                    "objectType": policy,
    ...
    def get_references(self, obj):
        ...
        for key, prop in getattr(self.module, "property_descriptions", {}).items():
            ...
            if (
                isinstance(syntax, udm_syntax.UDM_Objects)
                and syntax.key == "dn"
                and len(syntax.udm_modules) == 1
            ):
        ...
        return references + [
            dict(ref, property="__policies")
            for ref in self.get_policy_references(obj.dn)
        ]
    ...
    def flavor(self):
        ...
        for module in [x for x in udm_modules.modules.keys() if x.startswith(base)]:
            mod = UDM_Module(
                module, ldap_connection=ldap_connection, ldap_position=ldap_position
            )
    ...
    def _object_property_filter(
        ...
        if object_property in [None, "None"]:
            ...
            if object_property_value not in [None, "*"]:
                ret = "(|%s)" % "".join(
                    udm_syntax.ISyntax.get_object_property_filter(
                        attr, object_property_value, allow_asterisks
                    )
                    for attr in self.default_search_attrs
                )
        else:
            prop = self.module.property_descriptions.get(object_property)
            syn = prop.syntax if prop else udm_syntax.ISyntax
            ret = syn.get_object_property_filter(
                object_property, object_property_value, allow_asterisks=allow_asterisks
            )
    ...
    def _append_hidden_filter(self, ret):
        if (
            self.module is None
            or self.module.property_descriptions.get("objectFlag") is None
        ):
            return ret
...
def ldap_dn2path(ldap_dn, include_rdn=True):
    ...
    return "%s:/%s" % (
        ".".join(explode_dn(ldap_base, True)),
        "/".join(reversed(rel_path)),
    )
...
def get_obj_module(flavor, ldap_dn, ldap_connection=None, ldap_position=None):
    attr = ldap_connection.get(
        ldap_dn, ["*", "+"]
    )  # TODO: we should use module.object._ldap_attributes() here but we don't have the module yet
...
def _get_module(
    flavor, ldap_dn, attributes=None, ldap_connection=None, ldap_position=None
):
    if flavor is None or flavor == "navigation":
        base = None
    else:
        base, name = split_module_name(flavor)
    modules = udm_modules.objectType(
        None, ldap_connection, ldap_dn, attributes, module_base=base
    )
    ...
    MODULE.error(
        "Identified modules %r for %s (flavor=%s) does not have a relating UDM module."
        % (modules, ldap_dn, flavor)
    )
...
def list_objects(container, object_type=None, ldap_connection=None, ldap_position=None):
    ...
    for dn, attrs in result:
        ...
        for mod in modules:
            module = UDM_Module(
                mod, ldap_connection=ldap_connection, ldap_position=ldap_position
            )
            if module.module:
                break
        ...
        if not module.module:
            MODULE.process(
                "The UDM module %r could not be found. Ignoring LDAP object %r"
                % (modules[0], dn)
            )
            continue
        ...
        try:
            ...
            try:
                ...
                MODULE.error(
                    "Could not load object %r (%r) exception: %s"
                    % (dn, module.module, traceback.format_exc())
                )
...
def search_syntax_choices_by_key(syn, key, ldap_connection, ldap_position):
    if issubclass(syn.__class__, udm_syntax.UDM_Objects):
        if syn.key == "dn":
            try:
                return read_syntax_choices(
                    syn,
                    {"scope": "base", "container": key},
                    ldap_connection=ldap_connection,
                    ldap_position=ldap_position,
                )
        ...
        if syn.key is not None:
            ...
            if match:
                ...
                options = {
                    "objectProperty": attr,
                    "objectPropertyValue": key,
                    "allow_asterisks": False,
                }
                return read_syntax_choices(
                    syn,
                    options,
    ...
    return read_syntax_choices(
        syn, ldap_connection=ldap_connection, ldap_position=ldap_position
    )
...
def info_syntax_choices(syn, options=None, ldap_connection=None, ldap_position=None):
    if issubclass(syn.__class__, udm_syntax.UDM_Objects):
        ...
        for udm_module in syn.udm_modules:
            module = UDM_Module(
                udm_module, ldap_connection=ldap_connection, ldap_position=ldap_position
            )
            ...
            if filter_s is not None:
                try:
                    size += len(
                        module.search(filter=filter_s, simple=not syn.use_objects)
                    )
...
def read_syntax_choices(syn, options=None, ldap_connection=None, ldap_position=None):
    ...
    options["sizelimit"] = int(
        ucr.get("directory/manager/web/sizelimit", "2000") or 2000
    )
    if "$dn$" in options:
    ...
    if "container" in options:
        options["base"] = (
            options.pop("container") if options["container"] != "all" else None
        )
    if "objectProperty" in options:
        options["property"] = (
            options.pop("objectProperty")
            if options["objectProperty"] != "None"
            else None
        )
...
if __name__ == "__main__":
    set_bind_function(
        lambda lo: lo.bind(
            "uid=Administrator,cn=users,%s" % (ucr["ldap/base"],), "univention"
        )
    )

...
class LicenseImport(ldif.LDIFParser):
    ...
    def check(self, base):
        ...
        if not self.dn.endswith(base) and self.base.lower() in (
        ...
        if not self.dn.endswith(base) or not dnWithoutBase.endswith("cn=univention,"):
            raise LicenseError(
                _(
                    "The LDAP base of the license does not match the LDAP base of the UCS domain (%s)."
                )
                % base
            )

        # check LDAP base
        if self.base.lower() not in [
            base.lower(),
            "free for personal use edition",
            "ucs core edition",
        ]:
...
def check_license(ldap_connection, ignore_core_edition=False):
    try:
        ...
        raise LicenseError(
            _("License not found. During this session add and modify are disabled.")
        )
        ...
        raise LicenseError(
            _(
                "You have too many client accounts for your license. During this session add and modify are disabled."
            )
        )
    except udm_errors.licenseServers:  # UCS license v2
        raise LicenseError(
            _(
                "You have too many server accounts for your license. During this session add and modify are disabled."
            )
        )
        ...
        raise LicenseError(
            _(
                "You have too many desktop accounts for your license. During this session add and modify are disabled."
            )
        )
        ...
        raise LicenseError(
            _(
                "Your license is not valid for your LDAP-Base. During this session add and modify are disabled."
            )
        )
        ...
        raise LicenseError(
            _(
                "Your license does not allow modifications. During this session add and modify are disabled."
            )
        )
    except udm_errors.freeForPersonalUse:
        raise LicenseError(
            _(
                'You are currently using the "Free for personal use" edition of Univention Corporate Server.'
            )
        )

...
![Version: 0.44.1](https://img.shields.io/badge/Version-0.44.1-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square)
...
| global.nats | object | `{"auth":{"admin":{"existingSecret":{"keyMapping":{"adminPassword":null},"name":null}},"dispatcher":{"existingSecret":{"keyMapping":{"dispatcherPassword":null},"name":null},"key":"NATS_PASSWORD","name":null},"prefill":{"existingSecret":{"keyMapping":{"prefillPassword":null},"name":null},"key":"NATS_PASSWORD","name":null},"provisioningApi":{"existingSecret":{"keyMapping":{"provisioningApiPassword":null},"name":null},"key":"NATS_PASSWORD","name":null},"udmListener":{"existingSecret":{"keyMapping":{"udmListenerPassword":null},"name":null},"key":"NATS_PASSWORD","name":null},"udmTransformer":{"existingSecret":{"keyMapping":{"udmTransformerPassword":null},"name":null},"key":"NATS_PASSWORD","name":null}},"connection":{"host":"","port":""}}` | Define configuration regarding nats connectivity. |
...
| nats | object | `{"affinity":{"enabled":true},"bundled":true,"config":{"authorization":{"enabled":true,"users":[{"password":"$NATS_PASSWORD","permissions":{"publish":">","subscribe":">"},"user":"admin"},{"password":"$NATS_PROVISIONING_API_PASSWORD","permissions":{"publish":">","subscribe":">"},"user":"api"},{"password":"$NATS_DISPATCHER_PASSWORD","permissions":{"publish":">","subscribe":">"},"user":"dispatcher"},{"password":"$NATS_UDM_TRANSFORMER_PASSWORD","permissions":{"publish":">","subscribe":">"},"user":"udmtransformer"},{"password":"$NATS_PREFILL_PASSWORD","permissions":{"publish":">","subscribe":">"},"user":"prefill"},{"password":"$NATS_UDM_LISTENER_PASSWORD","permissions":{"publish":">","subscribe":">"},"user":"udmlistener"}]},"cluster":{"replicas":3},"extraConfig":{"max_payload":"16MB"},"jetstream":{"enabled":true}},"connection":{"host":"","port":"","tls":{"caFile":"/certificates/ca.crt","certFile":"/certificates/tls.crt","enabled":false,"keyFile":"/certificates/tls.key"}},"extraEnvVars":[{"name":"NATS_PASSWORD","valueFrom":{"secretKeyRef":{"key":"{{ include \"common.secrets.key\" (dict \"existingSecret\" .Values.global.nats.auth.admin.existingSecret \"key\" \"adminPassword\") }}","name":"{{ .Values.global.nats.auth.admin.existingSecret.name }}"}}},{"name":"NATS_PROVISIONING_API_PASSWORD","valueFrom":{"secretKeyRef":{"key":"{{ required \".Values.global.nats.auth.provisioningApi.existingSecret.keyMapping.provisioningApiPassword is required\" (include \"common.secrets.key\" (dict \"existingSecret\" .Values.global.nats.auth.provisioningApi.existingSecret \"key\" \"provisioningApiPassword\")) }}","name":"{{ .Values.global.nats.auth.provisioningApi.existingSecret.name }}"}}},{"name":"NATS_DISPATCHER_PASSWORD","valueFrom":{"secretKeyRef":{"key":"{{ include \"common.secrets.key\" (dict \"existingSecret\" .Values.global.nats.auth.dispatcher.existingSecret \"key\" \"dispatcherPassword\") }}","name":"{{ .Values.global.nats.auth.dispatcher.existingSecret.name }}"}}},{"name":"NATS_UDM_TRANSFORMER_PASSWORD","valueFrom":{"secretKeyRef":{"key":"{{ include \"common.secrets.key\" (dict \"existingSecret\" .Values.global.nats.auth.udmTransformer.existingSecret \"key\" \"udmTransformerPassword\") }}","name":"{{ .Values.global.nats.auth.udmTransformer.existingSecret.name }}"}}},{"name":"NATS_PREFILL_PASSWORD","valueFrom":{"secretKeyRef":{"key":"{{ include \"common.secrets.key\" (dict \"existingSecret\" .Values.global.nats.auth.prefill.existingSecret \"key\" \"prefillPassword\") }}","name":"{{ .Values.global.nats.auth.prefill.existingSecret.name }}"}}},{"name":"NATS_UDM_LISTENER_PASSWORD","valueFrom":{"secretKeyRef":{"key":"{{ include \"common.secrets.key\" (dict \"existingSecret\" .Values.global.nats.auth.udmListener.existingSecret \"key\" \"udmListenerPassword\") }}","name":"{{ .Values.global.nats.auth.udmListener.existingSecret.name }}"}}}],"nameOverride":"provisioning-nats","nats":{"image":{"registry":"docker.io"}},"natsBox":{"image":{"registry":"docker.io"}},"reloader":{"image":{"registry":"docker.io"}}}` | NATS server settings. |

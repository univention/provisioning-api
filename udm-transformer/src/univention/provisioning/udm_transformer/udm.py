...
class UDMMessagingService(univention.admin.uldap.access):
    ...
    async def send_event(self, new_obj: dict, old_obj: dict, ts: datetime) -> None:
    ...
    async def handle_changes(self, new_obj, old_obj, ts: datetime) -> None:
        old = {}
        if old_obj:

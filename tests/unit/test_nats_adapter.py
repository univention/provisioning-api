...
class TestNatsMQAdapter:
    ...
    async def test_add_message_new_stream(self, mock_nats_mq_adapter):
        ...
        mock_nats_mq_adapter._js.add_stream.assert_called_once_with(
            name=NatsKeys.stream(SUBSCRIBER_NAME), subjects=[SUBSCRIBER_NAME]
        )
        mock_nats_mq_adapter._js.add_consumer.assert_called_once_with(
            NatsKeys.stream(SUBSCRIBER_NAME),
    ...
    async def test_delete_stream_not_found(self, mock_nats_mq_adapter):
        ...
        mock_nats_mq_adapter._js.delete_stream.assert_not_called()
        assert result is None
    ...
    async def test_subscribe_to_queue(self, mock_nats_mq_adapter):
        result = await mock_nats_mq_adapter.subscribe_to_queue(SUBSCRIBER_NAME)

        mock_nats_mq_adapter._nats.subscribe.assert_called_once_with(
            SUBSCRIBER_NAME, cb=mock_nats_mq_adapter.cb
        )
        assert result is None

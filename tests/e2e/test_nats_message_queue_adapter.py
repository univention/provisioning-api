...
@pytest.fixture
async def test_stream(nats_mq_adapter: NatsMQAdapter):
...
@pytest.fixture
async def test_consumer(nats_mq_adapter: NatsMQAdapter, test_stream: str):
    await nats_mq_adapter.ensure_consumer(test_stream)
    yield True
    await nats_mq_adapter.delete_consumer(test_stream)


async def test_nats_connection(nats_mq_adapter: NatsMQAdapter):
    assert nats_mq_adapter


async def test_ensure_stream(nats_mq_adapter: NatsMQAdapter, test_stream):
    await nats_mq_adapter.ensure_stream(test_stream, False)
    await nats_mq_adapter.ensure_stream(test_stream, False)
    await nats_mq_adapter.ensure_stream(test_stream, False)


async def test_ensure_stream_with_filter(nats_mq_adapter: NatsMQAdapter):
    stream_name = "just_testing"
    await nats_mq_adapter.ensure_stream(stream_name, False, ["just_testing"])
    await nats_mq_adapter.delete_stream(stream_name)


async def test_ensure_stream_with_conflicting_filters(nats_mq_adapter: NatsMQAdapter):
    stream_name = "just_testing"
    await nats_mq_adapter.ensure_stream(stream_name, False, ["just_testing"])
...
async def test_update_stream_subject(nats_mq_adapter: NatsMQAdapter, test_stream):
...
async def test_ensure_consumer_with_name(nats_mq_adapter: NatsMQAdapter, test_stream: str, test_consumer):
    await nats_mq_adapter.ensure_consumer(test_stream)
    await nats_mq_adapter.ensure_consumer(test_stream)

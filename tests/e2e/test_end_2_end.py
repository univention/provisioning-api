...
def connect_ldap_server():
    server = ldap3.Server(udm_producer_settings.ldap_server_uri)
    connection = ldap3.Connection(
        server,
        udm_producer_settings.ldap_host_dn,
        udm_producer_settings.ldap_password,
    )
    connection.bind()
    return connection
...
async def test_workflow(provisioning_base_url):
    ...
    response = requests.post(
        f"{provisioning_base_url}{internal_app_path}{admin_api_prefix}/subscriptions",
        json={
            "name": name,
        ...
        auth=(app_settings.admin_username, app_settings.admin_password),
    )
